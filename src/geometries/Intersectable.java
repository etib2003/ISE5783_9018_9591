...
public abstract class Intersectable {
	...
	public final List<GeoPoint> findGeoIntersections(Ray ray) {
		return bbox != null && !bbox.intersect(ray) ? null : findGeoIntersectionsHelper(ray);
	}
	...
	/**
	 * The AABB class represents an Axis-Aligned Bounding Box. It is used to enclose
	 * objects and determine intersection in computer graphics and collision
	...
	public static class AABB {
		...
		/**
		 * Constructs an AABB with the given minimum point, maximum point, and center
		 * point. Expands the AABB slightly using the DELTA constant.
		 *
		...
		/**
		 * Constructs an AABB with the given minimum and maximum points. Calculates the
		 * center point and expands the AABB using the DELTA constant.
		 *
		 * @param min The minimum point of the AABB.
		 * @param max The maximum point of the AABB.
		 */
		public AABB(Point min, Point max) {
			...
			this.min = min;
			this.max = max;
			center = new Point((min.getX() + max.getX()) / 2, (min.getY() + max.getY()) / 2,
					(min.getZ() + max.getZ()) / 2);
		}

		/**
		 * Checks if the AABB intersects with the given Ray within the specified maximum
		 * distance.
		 *
